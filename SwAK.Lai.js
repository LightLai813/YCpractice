...
var SwAK = (function(){
    ...
    var _userAgent = (navigator.userAgent || navigator.vendor || window.opera).toLowerCase();
    var _browserAndDeivce = {
    ...
    function showLoading(container, size, type) {
        var scale = size || 1;
    ...
    //public
    return {
        rndFloat:rndFloat,          // random float => {input:(min, man), output:float}
        rndInt:rndInt,              // random integer => {input:(min, man), output:integer}
        angel2P:angel2P,            // get two points angel => {input:(bp,ep), output: angel}
        ...
        showLoading : showLoading,  // show loading => {input:(container, scale, type), output:null}
        hideLoading : hideLoading,  // hide loading => {input:null, output:null}
        preloadImg: preloadImg,     //preload images => {input:(imgArr,callback), output:null}
    };
}());
