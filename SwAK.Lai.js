/* SwAK Tool v0.1.6 ©Yuchun Lai  */
const SwAK = (function(){
    return {
        rndFloat: (min, max)=>{     // 亂數浮點數
        ...
        angel2P: (bp,ep)=>{         // 兩點取夾角
            let res=(Math.atan2(ep.y-bp.y,ep.x-bp.x))/Math.PI*180;
            return (res>=0 && res <=180)?res+=90:((res<0 && res>=-90)? res+=90: res+=450);
        },
        urlParams: (function(){     // 網址參數 
            const params = {};
            ...
            if (url.indexOf('?') != -1) {
                ...
                for (let i in ary) {
                    ...
                    const value = decodeURIComponent(ary[i].split('=')[1]);
                    params[key] = value;
                }
            }
            return params;
        ...
        ua: (function(){            // 使用者裝置各類資訊
            const _userAgent = (navigator.userAgent || navigator.vendor || window.opera).toLowerCase();
            return {
                mobile: (function(a) {return ((/android.+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|pad|p(ixi|re)\/|plucker|pocket|psp|symbian|treo|up\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino/i.test(a) || /1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|e\-|e\/|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(di|rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|xda(\-|2|g)|yas\-|your|zeto|zte\-/i.test(a.substr(0, 4))) ? true: false); }(_userAgent)),
                ipad: _userAgent.match(/ipad/i) == "ipad",
                iphone: _userAgent.match(/iphone os/i) == "iphone os",
                iOSversion: _userAgent.substring(_userAgent.indexOf("iphone os")+10, _userAgent.length).split(" ")[0],
                midp: _userAgent.match(/midp/i) == "midp",
                Uc7: _userAgent.match(/rv:1.2.3.4/i) == "rv:1.2.3.4",
                Uc: _userAgent.match(/ucweb/i) == "ucweb",
                iOS: (_userAgent.indexOf('iphone') > -1 || _userAgent.indexOf('ipad') > -1),
                android: _userAgent.match(/android/i) == "android",
                CE: _userAgent.match(/windows ce/i) == "windows ce",
                WM: _userAgent.match(/windows mobile/i) == "windows mobile",
                Line: _userAgent.match(/line/i) == "line",
                Facebook: _userAgent.match(/fb/i) == "fb",
                wechat: _userAgent.search("micromessenger") > -1,
                ie: _userAgent.search("msie") > -1,
                ie11: _userAgent.search("trident") > -1,
                firefox: _userAgent.search("firefox") > -1,
            }
        }()),
        cData: {                    // 資料正確性比對
        ...
        cookie: {
            write: param=>{
                let cookieContent = `${param.name}=${param.value}`;
        
                if (param.hours) {
                    var date = new Date();
                    date.setTime(date.getTime() + (param.hours * 60 * 60 * 1000));
                    cookieContent += `; expires=${date.toGMTString()}`
                } 
        
                if (param.domain) {
                    cookieContent += `; domain=${param.domain}`;
                } 
                cookieContent += "; path=/";
        ...
        },        // write cookie => {input:(name, value, hours), output:null}
        loading: {
            show: (container, size, type)=>{
                const scale = size || 1;
                const bgColor = type && type == 'light' ? 'rgba(250, 250, 250, 0.9)' : 'rgba(0, 0, 0, 0.9)';
                const circleColor = type && type == 'light' ? '#555555' : '#CCCCCC';
            ...
            hide: ()=>{ 
                document.getElementById('SwAK_Loading').remove(); 
            }
        },
        preloadImg: (imgArr,callback)=>{
            let onloadCount = 0;
            const image = [];
    
            for(let i=0, imgUrl; imgUrl=imgArr[i];i++){
                image[i] = new Image();
                image[i].onload = ()=>{
